...
- **`agent/*`——运行时实时表面。** 始终携带活的 `Agent`。拦截 waterfall（瀑布式事件）（`agent/pre-step`、`agent/request`、`agent/request-error`、`agent/assistant-delivery` 与 `agent/final-candidate`）负责变换、拒绝、恢复或控制响应发布；`agent/request-starting` 同步标记每次模型请求尝试；awaited `agent/turn-stopping` 观察停止边界；瞬态 emit 报告生命周期、状态、inbox 的插入、领取和丢弃，以及错误。轮次和步骤边界不在此处——它们是持久的会话事件，从 `session/event` 读取；普通 token 流（`assistant/chunk`）、延迟私有 staging 事件，以及轮次中途以 `user/message` 呈现的 steering（中途引导）同理。
- **`tools/*`——工具注册表与执行流水线。**
