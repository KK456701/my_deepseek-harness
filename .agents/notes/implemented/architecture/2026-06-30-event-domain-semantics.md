...
- **`agent/*` — the LIVE runtime surface.** Always carries the live `Agent`. Interception waterfalls (`agent/pre-step`, `agent/request`, `agent/request-error`, `agent/assistant-delivery`, and `agent/final-candidate`) transform, reject, recover, or control response publication; `agent/request-starting` synchronously marks each model-request attempt; awaited `agent/turn-stopping` observes the stop boundary; transient emits report lifecycle, status, inbox insertion/claim/discard, and errors. Turn and step BOUNDARIES are NOT here — they are durable session events read off `session/event`, as are ordinary token streams (`assistant/chunk`), deferred private staging events, and mid-turn steering (a `user/message`).
