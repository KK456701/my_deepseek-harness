...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    const toolCategory = dialog.locator('[data-category-filter="tools"]')
    await toolCategory.click()
    expect(await toolCategory.getAttribute('aria-pressed')).toBe('true')
    const visibleToolPlugins = dialog.locator('[data-plugin-category="tools"]')
    expect(await visibleToolPlugins.count()).toBeGreaterThan(0)
    expect(await dialog.locator('[data-plugin-entry]:not([data-plugin-category="tools"])').count()).toBe(0)
    await dialog.locator('[data-category-filter="all"]').click()
    expect(await dialog.locator('[data-plugin-entry]').count()).toBe(expectedPluginCount)
