...
第一方交互式持久化 Provider 只拥有 `interactive` 和 `subagent` Session。其直接 API 会拒绝 capability 私有的 `maintenance` Session，事件协调器也会忽略这类 Session；对应 capability 必须提供独立的持久审计日志。普通 Session 的列出、搜索、恢复、导出和保留策略因此不会暴露维护模型调用。
