...
A pristine database initializes directly at schema 18, with required `purpose` (`interactive`, `subagent`, or `maintenance`). Older schemas, foreign application identities, non-pristine unversioned databases, and incompatible schema objects reject during normal open. Every statement and fixed pragma lives in a packaged `.sql` resource; values use SQLite parameters and runtime code never assembles query text.

An explicit offline upgrade is available from the two integration inputs: schema 16 with `purpose`, and schema 17 with nullable `origin`. Stop all writers and back up the database first, then run `pnpm exec tsx scripts/upgrade-session-sqlite.ts --source <absolute-source> --output <new-absolute-output> --confirm-stopped-and-backed-up`. It never replaces the source or an existing destination. It verifies schema ownership, converts only known origins, preserves IDs, lineage and revisions, and compares logical-event hashes before committing. Schema-17 packed rows remain byte-identical. Unknown or corrupt input rejects instead of producing empty history. Inspect the report before changing deployment configuration. JSONL and memory databases are outside this operation.
