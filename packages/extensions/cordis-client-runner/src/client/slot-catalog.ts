...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, context, manual-compaction, model-retry, steering, task-extraction-failure, task-input-receipt, task-review-test, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      ...
      'experimental-ui-task-review TestRecordRow key \'task-review-test\'',
      'experimental-ui-task-review InputReceiptRow key \'task-input-receipt\'',
      'experimental-ui-task-review ExtractionFailureRow key \'task-extraction-failure\'',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-memory MemoryControlsAction id \'memory-controls\'',
  ...
  {
    key: 'sidebar.workspaces.header.actions',
    kind: 'list',
    scope: 'root',
    summary: 'Optional actions placed before the workspace view and add controls.',
    doc: 'Optional actions placed before the workspace view and add controls.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'sidebar.workspaces\' (client-ui-workspace), so it exists while that entry is mounted',
    occupants: [
      'client-ui-memory MemoryArea id \'memory\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'sidebar.workspaces.header.actions\', () => ctx.slots.register(\n      { name: \'sidebar.workspaces.header.actions\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
