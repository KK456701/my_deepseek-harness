...
该插件向设置贡献只读的 **Skill** 页面。页面列出为当前选中普通会话解析出的目录，因此工作区与 agent 预设的 skill 范围和 `/` 菜单一致；页面共用同一份按会话拉取缓存。每一项显示 `/名称`、简介、可选的 `whenToUse`，以及模型能否调用该 skill；搜索在本地匹配这三个文本字段。未选择会话时，页面提示先打开会话；空目录会明确显示，RPC 失败时提供重试但不暴露传输细节。安装、删除和编辑不属于该页面。
