...
每个启用记忆感知的助手请求都有紧凑的“长期记忆上下文”行：首次注入、沿用、更新或未注入。其 Input 检查器从该请求已记录的 header 读取实际摘要，并展示已记录的 generation、字节数、条目数和跳过原因。旧日志使用已有系统提示词中的记忆段；没有记录的未注入原因保持未知。这是上下文观察，不是虚构的记忆工具调用，也不增加后台流水线面板。

扩展可以贡献独立模型调用记录，明确请求身份、所属 Turn、生命周期状态以及纯数据检查器区域。这些记录不会变成工具或 Worker 请求。流式详情按动画帧发布；会话中断边界会关闭未结束的展示状态，不补造提供方结果。
