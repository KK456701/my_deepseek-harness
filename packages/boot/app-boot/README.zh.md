...
| `createProfilePathResolver(profileDir)` | 为已经验证的 profile 目录创建 `profilePath(...segments)` Loader 表达式解析器；绝对、空或逃逸 segment 会在插件解析到 profile 外存储之前失败 |
