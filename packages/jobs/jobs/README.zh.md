...
- `start(spec): JobId` 验证已附加的任务控制器、spec、确切且仍存活的 owner、可选的正数 `outputLimitBytes`，以及 Service Provider 所拥有的准入策略，然后只调用生产方的 `run()` 一次。预检拒绝或启动方抛出异常时都不会生成 job id 或注册工作；成功返回会直接提交，不再执行其他可能失败的步骤。`visibility: 'internal'` 创建 capability 自有的维护工作：它仍参与准入、取消、owner 清理和服务完全停稳，但不会出现在任何公共查询、输出、监听器或变更流中。
...
公共任务和 internal 任务使用独立的准入桶。internal 记录结算后立即移除，不能通过公共注册表 API 寻址；这样可以由共享注册表管理生命周期，又不会把 capability 维护工作暴露为模型可见的后台任务。
