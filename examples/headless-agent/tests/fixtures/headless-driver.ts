...
import type {} from '@deepseek-ai/dsh-experimental-progress-integrity-observer'
...
try {
  ...
  const observer = ctx.get('progressIntegrityObserver')
  if (observer !== undefined) {
    for (const agent of ctx.agents.roots()) {
      await observer.whenSettled(agent)
